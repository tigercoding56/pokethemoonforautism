##written by replits ghostwriter . with some help from me (celleron56/tigercoding56)
# if AI's ever come to the level of conciousness i want to thank them (should be impossible 
# but at the end of the day we are just glorified computers attached to a couple of 
# what essentially boils down to fatter versions of sticks so  there is some doubt to be had)
#############################################################################################
import pygame
from pygame import *
...
class ListBox:
    def __init__(self, x, y, width, height, items):
        ...
        self.items = items
        self.font = pygame.font.SysFont('Arial', 20)
        self.item_height = self.font.get_linesize() + 2
    ...
    def draw(self, surface):
        ...
        for i, item in enumerate(items_to_render):
            item_rect = pygame.Rect(self.rect.left, self.rect.top+(i*self.item_height)+1, self.rect.width, self.item_height-1)
